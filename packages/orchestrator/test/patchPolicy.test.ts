...
describe("Autofix patch policy", () => {
  ...
  it("rejects empty, oversized, and likely-secret replacements", () => {
    ...
    expect(() => validatePatchProposals({ proposals: [{ ...proposal, replacementContent: `api_key = "${["abcdefghij", "klmnop", "1234"].join("")}"` }], sources: [{ path: proposal.path, content: source, contentHash: hash }], acceptedFindingIds: accepted })).toThrow("patch_potential_secret");
  });
