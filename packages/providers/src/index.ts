...
// AbortSignal.timeout rejects with a TimeoutError DOMException, which is not a ProviderError.
function isTimeout(error:unknown){const name=(error as {name?:string}|null)?.name;return name==="TimeoutError"||name==="AbortError"}
...
export function assertStrictSchema(schema:JsonSchema){const visit=(node:unknown)=>{if(!node||typeof node!=="object"||Array.isArray(node))return;const value=node as Record<string,unknown>;if(value.type==="object"){if(value.additionalProperties!==false)throw new ProviderError("malformed_response");const properties=value.properties&&typeof value.properties==="object"&&!Array.isArray(value.properties)?Object.keys(value.properties):[],required=Array.isArray(value.required)?value.required:[];if(properties.some(key=>!required.includes(key)))throw new ProviderError("malformed_response")}if(value.properties&&typeof value.properties==="object")for(const child of Object.values(value.properties))visit(child);if(value.items)visit(value.items)};visit(schema)}
