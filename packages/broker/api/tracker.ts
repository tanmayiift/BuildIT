...
export const POST = async (request: Request) => {
  try {
    ...
    return handleTrackerFetch(request, { grantSecret: Buffer.from(required("TRACKER_GRANT_SECRET"), "base64url"), consume: (id, expiresAt) => replay.consume(id, expiresAt), kms });
  } catch {
