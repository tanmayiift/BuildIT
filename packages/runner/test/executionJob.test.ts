...
describe("durable execution jobs", () => {
  ...
  it("advances one stage and replays the same callback without duplicating it", () => {
    ...
    const checkpoint = { requestKey: "prepare-1", expectedVersion: 1, expectedStage: "prepare" as const, nextStage: "install" as const, cursor: "context-artifact", artifactIds: ["artifact-1"], now: 1_002, durationMs: 100 };
    const first = applyExecutionCheckpoint(claimed, checkpoint);
    expect(first.replayed).toBe(false);
    expect(first.job).toMatchObject({ stage: "install", stateVersion: 2, status: "checkpointed", artifactIds: ["artifact-1"] });
  ...
  it("refuses stale or out-of-order checkpoints and never advances after cancellation", () => {
    const created = createExecutionJob(input);
    const claimed = claimExecutionJob(created, "worker-a", 1_001);
    expect(() => applyExecutionCheckpoint(claimed, { requestKey: "wrong-version", expectedVersion: 0, expectedStage: "prepare", nextStage: "install", cursor: "x", now: 1_002, durationMs: 1 })).toThrow("execution_checkpoint_conflict");
    expect(() => applyExecutionCheckpoint(claimed, { requestKey: "wrong-stage", expectedVersion: 1, expectedStage: "prepare", nextStage: "scanners", cursor: "x", now: 1_002, durationMs: 1 })).toThrow("execution_stage_order_invalid");
    const cancelled = cancelExecutionJob(claimed, 1_002);
    expect(cancelled.status).toBe("cancelled");
    expect(() => applyExecutionCheckpoint(cancelled, { requestKey: "late", expectedVersion: 1, expectedStage: "prepare", nextStage: "install", cursor: "x", now: 1_003, durationMs: 1 })).toThrow("execution_job_cancelled");
  });
  ...
  it("records a failed stage while retaining its checkpoint and does not turn it into success", () => {
    ...
    const result = applyExecutionCheckpoint(claimed, { requestKey: "failed-install", expectedVersion: 1, expectedStage: "prepare", nextStage: "install", cursor: "install-failed", now: 1_002, durationMs: 200, failureCode: "runner_unavailable" });
    expect(result.job).toMatchObject({ status: "failed", stage: "install", cursor: "install-failed", failureCode: "runner_unavailable" });
    const retried = claimExecutionJob(result.job, "worker-b", 1_003);
    expect(retried).toMatchObject({ status: "running", stage: "install", cursor: "install-failed", attempt: 2 });
