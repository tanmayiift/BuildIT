...
export function defaultExecutionPlans(manager:PackageManager){return{install:{...createNamedPlan({planId:"install",manager,origin:"built_in",required:true}),timeoutMs:150_000},checks:["test","lint","typecheck"].map(planId=>({...createNamedPlan({planId:planId as "test"|"lint"|"typecheck",manager,origin:"built_in",required:planId==="test"}),timeoutMs:planId==="test"?150_000:60_000}))}}
export function validatePlan(plan:CommandPlan){const expected=commands[plan.executable]?.[plan.planId];if(!expected||plan.args.length!==expected.length||plan.args.some((arg,index)=>arg!==expected[index])||plan.kind!==kinds[plan.planId])throw new Error("command_not_allowed");if(plan.origin!=="built_in"&&plan.origin!=="trusted_ref")throw new Error("untrusted_command_origin");if(plan.timeoutMs<1||plan.timeoutMs>1_200_000||plan.cpuLimit<1||plan.cpuLimit>4||plan.memoryMb<128||plan.memoryMb>8192||plan.outputBytes<1||plan.outputBytes>25_000_000||plan.fileBytes<1||plan.fileBytes>2_000_000_000)throw new Error("invalid_resource_limit");if((plan.planId==="install")!==(plan.network==="registry_only"))throw new Error("invalid_network_policy");return plan}
