...
const aacr = { instance_id: "acme__service@bbbbbbb", repo: "acme/service", base_commit: "a".repeat(40), head_commit: "b".repeat(40), clone_url: "https://github.com/acme/service.git", reference_comments: [{ path: "src/limit.ts", start_line: 4, end_line: 4, side: "right", text: "Equality bypasses the stated limit." }] };
...
describe("official benchmark contract adapters", () => {
  it("separates AACR task context from hidden review gold", () => {
    ...
    expect(adapted.gold.comments[0]?.text).toContain("Equality");
  ...
  it("fails closed on stale ranges, repository mismatch, or unpinned provenance", () => {
    expect(() => adaptAacrReview({ ...aacr, head_commit: "a".repeat(40) }, aacrSource)).toThrow("aacr_commit_range_invalid");
    expect(() => adaptAacrReview({ ...aacr, clone_url: "https://github.com/other/repo.git" }, aacrSource)).toThrow("aacr_clone_url_invalid");
  ...
  it("accepts both official SWE test-list encodings and rejects malformed labels", () => {
    ...
    expect(() => adaptAacrReview({ ...aacr, reference_comments: [] }, aacrSource)).toThrow("aacr_gold_invalid");
  });
});
