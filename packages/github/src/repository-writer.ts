...
export class GitHubRepositoryWriter {
  ...
  async upsertCheckRun(input: { name: string; headSha: string; conclusion?: "success" | "failure" | "neutral" | "action_required"; title: string; summary: string; detailsUrl?: string }) {
    ...
    const body = JSON.stringify({ name: input.name, head_sha: input.headSha, ...(input.conclusion ? { status: "completed", conclusion: input.conclusion } : { status: "in_progress" }), ...(detailsUrl ? { details_url: detailsUrl } : {}), output: { title: input.title, summary: input.summary } });
  ...
  async publishInlineFindings(input: { prNumber: number; headSha: string; marker: string;
    ...
    if (!Number.isInteger(input.prNumber) || input.prNumber < 1 || !/^[0-9a-f]{40}$/i.test(input.headSha)
      || !/^buildit-(?:review|autofix):(?:(?:[a-z]{1,16}-)?pr-[1-9]\d{0,9}|[A-Za-z0-9_|-]+:[0-9a-f]{40})$/.test(input.marker)) throw new Error("inline_findings_input_invalid");
  ...
  async upsertIssueComment(input: { prNumber: number; marker: string; body: string }) {
    if (!Number.isInteger(input.prNumber) || input.prNumber < 1 || !/^buildit-(?:review|autofix):(?:(?:[a-z]{1,16}-)?pr-[1-9]\d{0,9}|[A-Za-z0-9_|-]+:[0-9a-f]{40})$/.test(input.marker) || !input.body.trim()) throw new Error("comment_input_invalid");
