...
function tree(entries: unknown[], extra = {}) { return new Response(JSON.stringify({ sha, tree: entries, ...extra }), { status: 200 }); }
...
describe("exact repository content fetch", () => {
  it("fetches text blobs from the exact requested commit", async () => {
    const http = vi.fn(async (url: string | URL) => String(url).includes("/trees/")
      ? tree([{ path: "src/index.ts", type: "blob", sha: blobSha, size: 12 }])
      : blob("export {}\n"));
    ...
    expect(http.mock.calls[0]![0]).toContain(`/trees/${sha}?recursive=1`);
  });

  it("fails closed when GitHub truncates or changes the requested tree", async () => {
    await expect(new RepositoryContentClient(vi.fn(async () => tree([], { truncated: true }))).fetchExactCommit({ installationToken: "token", repositoryId: 7, commitSha: sha })).rejects.toThrow("github_tree_truncated");
    await expect(new RepositoryContentClient(vi.fn(async () => new Response(JSON.stringify({ sha: "c".repeat(40), tree: [] })))).fetchExactCommit({ installationToken: "token", repositoryId: 7, commitSha: sha })).rejects.toThrow("github_tree_sha_mismatch");
  });

  it("reports exclusions and budget omissions as partial coverage", async () => {
    const http = vi.fn(async (url: string | URL) => String(url).includes("/trees/") ? tree([
  ...
  it("rejects unsafe paths before fetching blobs", async () => {
    const http = vi.fn(async () => tree([{ path: "../secret", type: "blob", sha: blobSha, size: 1 }]));
  ...
  it("treats null-containing blobs as binary and never returns their content", async () => {
    const http = vi.fn(async (url: string | URL) => String(url).includes("/trees/") ? tree([{ path: "unknown.dat", type: "blob", sha: blobSha, size: 3 }]) : blob("a\0b"));
    const result = await new RepositoryContentClient(http).fetchExactCommit({ installationToken: "token", repositoryId: 7, commitSha: sha });
