...
test("public data handling states the current access boundary", async ({ page }) => {
  ...
  await expect(page.getByText(/What is still thin is the track record/i)).toBeVisible();
  await expect(page.getByText(/not enough to claim reliability on a codebase unlike those/i)).toBeVisible();
...
test("an unknown address is a real 404, and every real page still is not", async ({ request }) => {
  ...
  for (const path of ["/", "/reviews", "/notifications", "/setup/install", "/data-handling", "/pricing", "/scan", "/sign-in", "/account"]) {
