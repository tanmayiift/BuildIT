...
describe("observability release assets", () => {
  it("ships a valid source-free dashboard", () => {
    ...
    expect(dashboard.panels.map(panel => panel.title)).toEqual(expect.arrayContaining(["Failure ratio", "p95 latency by operation", "Review stages and outcomes"]));
  });

  it("alerts on failure, latency, silence, and safety boundaries", () => {
    const rules = read("observability/alerts.yml");
    for (const alert of ["BuildITHighFailureRate", "BuildITP95LatencyHigh", "BuildITTelemetrySilent", "BuildITCriticalBoundaryFailure"]) expect(rules).toContain(`alert: ${alert}`);
  });
