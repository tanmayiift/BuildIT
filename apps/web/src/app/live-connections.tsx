...
function profileLabel(profile: string | undefined) {
...
export function recentLoginRequired(reason: unknown) {
  ...
  return <article className="repository-row" aria-label={`Repository policy for ${fullName}`}>
    ...
    <div className="repository-control-group">
      ...
      <label className="repository-policy">
        ...
        {canManage ? <select className="repository-policy-select" aria-label={`Changelog on merge for ${fullName}`} value={repository.changelogOnMerge ? "on" : "off"} disabled={saving} onChange={event => void onSave({ changelogOnMerge: event.target.value === "on" })}>
      ...
      <label className="repository-policy">
        ...
        {canManage ? <select className="repository-policy-select" aria-label={`Inline comment level for ${fullName}`} value={repository.reviewProfile ?? "balanced"} disabled={saving} onChange={event => void onSave({ reviewProfile: event.target.value as "quiet" | "balanced" | "thorough" })}>
          <option value="quiet">Only what blocks</option>
...
export function PermissionReceipt() {
  ...
  // `!connection` means the workspace query is still in flight. Claiming nothing is connected
