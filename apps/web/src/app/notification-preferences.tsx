...
type Preferences = { emailEnabled: boolean; emailOptedIn?: boolean; deliveryAvailable: boolean; captureAvailable?: boolean; digestMode: "immediate" | "daily"; mutedRepositoryIds: string[]; updatedAt: number | null; recipient: { state: "verified"; maskedEmail: string } | { state: "verification_required" } };
...
export function NotificationPreferences() {
  ...
  const email = notificationEmailState(saved), controlsAvailable = saved.deliveryAvailable || saved.captureAvailable, optedIn = saved.emailOptedIn ?? saved.emailEnabled;
  return <>
    <section className={`settings-list notification-settings${controlsAvailable?" has-actions":""}`} aria-label="Customer email status">
      <article className="setting-row"><div><strong>Customer review email</strong><p>{email.summary}</p></div><span className="status neutral">{email.status}</span>{controlsAvailable?<button type="button" disabled={working||(!optedIn&&saved.recipient.state!=="verified")} onClick={()=>void save({emailEnabled:!optedIn})}>{saved.captureAvailable ? (optedIn ? "Disable local capture" : "Enable local capture") : (optedIn ? "Turn off" : "Turn on")}</button>:null}</article>
      <article className="setting-row"><div><strong>{saved.captureAvailable ? "Capture recipient" : "Future recipient"}</strong><p>{email.recipient}</p></div><span className="recipient-scope">One member</span></article>
      ...
      {controlsAvailable?<article className="setting-row"><div><strong>{saved.captureAvailable ? "Capture timing" : "Delivery timing"}</strong><p>Immediate records each event. Daily groups routine updates at midnight UTC; budget and service failures remain immediate.</p></div><select aria-label={saved.captureAvailable ? "Local capture timing" : "Email delivery timing"} value={saved.digestMode} disabled={working||!saved.emailEnabled} onChange={event=>void save({digestMode:event.target.value as Preferences["digestMode"]})}><option value="immediate">Immediate</option><option value="daily">Daily digest</option></select></article>:null}
    </section>
    {controlsAvailable?<section className="settings-list" aria-label="Repository notification muting">{connection.repositories.map(repository=>{const muted=saved.mutedRepositoryIds.includes(repository.id);return <article className="setting-row" key={repository.id}><div><strong>{repository.owner}/{repository.name}</strong><p>{muted?"All repository notifications muted":"Notifications follow your delivery timing"}</p></div><button type="button" disabled={working} onClick={()=>void save({mutedRepositoryIds:muted?saved.mutedRepositoryIds.filter(id=>id!==repository.id):[...saved.mutedRepositoryIds,repository.id]})}>{muted?"Unmute":"Mute"}</button></article>})}</section>:null}
