...
const sample = {
  cancelled: { status: "cancelled", stage: "queue", action: "start_new_review", detail: "No requirements, issues, or checks were recorded before this review stopped.", evidence: "empty" },
...
export default async function Review({ params, searchParams }: { params: Promise<{ id: string }>; searchParams: Promise<{ tour?: string; state?: string }> }) {
  ...
  const chosen = stateFrom(state), current = sample[chosen], verdict = statusPresentation(current.status, false), next = nextActionPresentation(current.action, false);
  return <main className="content review-detail review-tour">
    ...
    <section className="verdict-card"><div className="verdict-message"><span className={`verdict-symbol ${verdict.tone}`} aria-hidden="true">{verdict.symbol}</span><div><span className={`status ${verdict.tone}`}>{verdict.label}</span><h1>{verdict.title}</h1><p>{current.detail}</p></div></div><div className="verdict-actions"><a className="button secondary" href="/reviews?tour=1">Back to queue</a></div></section>
    <section className="tour-scope" aria-label="Pinned review context"><span><small>Repository</small><strong>nexus/api</strong></span><span><small>Pull request</small><strong>#{id}</strong></span><span><small>Exact commit</small><code>a3f91c2</code></span><span><small>Review coverage</small><strong>Full</strong></span></section>
    <div className="next-action"><span className="next-mark" aria-hidden="true">→</span><div><small>What to do next</small><strong>{next.title}</strong><p>{next.detail}</p></div></div>
    <Journey stage={current.stage} />
    {current.evidence === "empty" ? <section className="evidence-empty"><span aria-hidden="true">◇</span><div><h2>No evidence yet</h2><p>BuildIT does not fill the page with guesses. Start a new review once repository access and safety checks are ready.</p></div></section> : null}
