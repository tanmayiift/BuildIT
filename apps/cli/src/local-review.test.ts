...
describe("BuildIT local CLI", () => {
  it("pins a clean commit, previews only allowlisted scripts, and returns stable JSON events", async () => {
    ...
    try {
      ...
      await expect(runLocalReview({ cwd: root, confirmed: true, emit: item => events.push(item), exec })).resolves.toMatchObject({ status: "checks_passed", exitCode: 0 });
  ...
  it("includes dirty scoped files, pins --base, ignores untrusted policy, and never writes", async () => {
    ...
    try {
      ...
      await expect(runLocalReview({ cwd: root, directory: "packages/billing", baseRef: "release/1", confirmed: true, emit: item => events.push(item), exec })).resolves.toMatchObject({ exitCode: 0 });
  ...
  it("prints a zero-cost no-upload plan and runs no repository command without consent", async () => {
    ...
    try {
      ...
      await expect(runLocalReview({ cwd: root, emit: item => events.push(item), exec })).resolves.toEqual({ status: "consent_required", exitCode: 3 });
      expect(events[1]?.data).toMatchObject({ estimatedProviderCostUsd: 0, uploadsFiles: false, requiresConfirmation: true });
      expect(events.at(-1)?.data).toMatchObject({ status: "consent_required", nextAction: "rerun_with_confirm_run", workingTreeModified: false });
      expect(calls.every(([file]) => file === "git")).toBe(true);
    } finally { await rm(root, { recursive: true, force: true }); }
  });
