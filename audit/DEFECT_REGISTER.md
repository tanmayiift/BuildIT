...
Independent audit, 2026-09-02. **No source file was modified** — the instruction for this
audit was "list issues and their fix", so every entry below is diagnosis + proposed patch,
not an applied change. Fix status is therefore `Not fixed (by instruction)` throughout.
