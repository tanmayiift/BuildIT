...
export const complete = internalMutation({
  args: { organizationId: v.id("organizations"), reviewId: v.id("reviews"), expectedHeadSha: v.string(), expectedGeneration: v.number(),
    ...
    coverage: v.union(v.literal("full"), v.literal("partial")), coverageGap: v.optional(coverageGap), unreadableSources: v.optional(v.object({ total: v.number(), unreadable: v.number(), summary: v.string(), nextStep: v.string() })), now: v.number() },
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(review._id, { status: "gathering_context", currentStage: "context", coverageLevel: args.coverage, coverageGap: args.coverageGap, unreadableSources: args.unreadableSources, updatedAt: args.now });
    return artifact._id;
  },
});
