...
const repository=v.object({githubRepositoryId:v.number(),owner:v.string(),name:v.string(),defaultBranch:v.string()});
export const attachUserInstallation=internalMutation({args:{userId:v.string(),githubUserId:v.number(),githubLogin:v.string(),installationId:v.number(),accountLogin:v.string(),accountId:v.number(),permissions:v.object({metadata:v.literal("read"),contents:v.union(v.literal("read"),v.literal("write")),pullRequests:v.literal("write"),issues:v.literal("read"),checks:v.union(v.literal("read"),v.literal("write"))}),repositories:v.array(repository),now:v.number()},handler:async(ctx,args)=>{
 ...
 for(const repo of args.repositories){const stored=existing.find(item=>item.githubRepositoryId===repo.githubRepositoryId);if(stored)await ctx.db.patch(stored._id,{owner:repo.owner,name:repo.name,defaultBranch:repo.defaultBranch,enabled:true,pausedAt:undefined,updatedAt:args.now});else await ctx.db.insert("repositories",{organizationId,installationId:installationDocId,githubRepositoryId:repo.githubRepositoryId,owner:repo.owner,name:repo.name,defaultBranch:repo.defaultBranch,enabled:true,autofixMode:"stacked",forkPolicy:"manual_review_only",indexState:"not_started",concurrencyLimit:1,createdAt:args.now,updatedAt:args.now})}
