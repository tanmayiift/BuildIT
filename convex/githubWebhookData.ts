...
export const materializeReview = internalMutation({
  args: { deliveryId: v.string(), organizationId: v.id("organizations"), repositoryId: v.id("repositories"),
    baseRef: v.string(), triggerActor: v.string(), actorPermission: v.union(v.literal("read"), v.literal("triage"), v.literal("write"), v.literal("maintain"), v.literal("admin")),
    now: v.number() },
  handler: async (ctx, args) => {
    const delivery = await ctx.db.query("webhookDeliveries").withIndex("by_delivery_id", q => q.eq("deliveryId", args.deliveryId)).unique();
    const repository = await ctx.db.get(args.repositoryId);
    if (!delivery || delivery.status !== "received" || !delivery.prNumber || !delivery.headSha || !delivery.baseSha
      || !delivery.triggerVerb || delivery.triggerVerb === "cancel" || delivery.reviewId
      || !repository || repository.organizationId !== args.organizationId || repository.githubRepositoryId < 1) throw new Error("review_request_not_materializable");
    ...
    if (!config || config.repositoryId !== repository._id || config.organizationId !== args.organizationId) {
      ...
      const configId = await ctx.db.insert("configRevisions", { organizationId: args.organizationId, repositoryId: repository._id,
        sourceCommitSha: delivery.baseSha, sourceRef: args.baseRef, contentHash, rulesDigest: contentHash,
        schemaVersion: "defaults-v1", validationState: "valid", provenance: "defaults_only",
        refProtectionState: "unverified", createdAt: args.now });
      await ctx.db.patch(repository._id, { configRevisionId: configId, updatedAt: args.now });
    ...
    const model = provider === "gemini" ? "gemini-2.5-pro" : provider === "openai" ? "gpt-5" : "claude-sonnet-4-5";
    const status = credential ? "queued" as const : "blocked" as const;
    const reviewId = await ctx.db.insert("reviews", { organizationId: args.organizationId, repositoryId: repository._id,
      githubRepositoryId: repository.githubRepositoryId, prNumber: delivery.prNumber, isFork: delivery.isFork ?? false,
      baseRef: args.baseRef, baseSha: delivery.baseSha, headSha: delivery.headSha, requiredCheckPolicy: "fail_closed",
      completedRoundCount: 0, patchAttemptCount: 0, diagnosticRunCount: 0, providerRetryCount: 0, commandRetryCount: 0,
      trigger: "github_comment", triggerVerb: delivery.triggerVerb, triggerActor: args.triggerActor,
      triggerActorPermission: args.actorPermission, mode, status, budgetLimit: 5, budgetConsumed: 0,
      statusReasonCode: credential ? undefined : "provider_credential_invalid", nextActionCode: credential ? "none" : "reconnect_provider",
      isStale: false, trustedRef: args.baseRef, trustedRefSha: delivery.baseSha, configRevisionId: config._id,
      configProvenance: "defaults_only", provider, model, modelVersion: "pinned-at-execution", promptVersion: "chain-v1",
      evalSetVersion: "buildit-eval-v1", coverageLevel: "limited", currentStage: "queue", executionGeneration: 0,
      queuePriority: 0, runnerImageVersion: "buildit-node22-v1", expiresAt: args.now + 7 * 86_400_000,
      createdAt: args.now, updatedAt: args.now });
    await ctx.db.insert("reviewLocks", { repositoryId: repository._id, prNumber: delivery.prNumber, headSha: delivery.headSha,
      mode, reviewId, createdAt: args.now });
