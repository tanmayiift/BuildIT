...
// Convex mutations run in its edge-compatible runtime. Keep this small,
// conservative calculation here instead of importing the Node-only review
// package. It intentionally uses a higher ceiling than any supported provider.
const conservativeModelCost = (inputTokens: number, outputTokens: number) => inputTokens * 15 / 1_000_000 + outputTokens * 75 / 1_000_000;
const conservativeStageCost = (inputBytes: number, maxOutputTokens: number) => conservativeModelCost(inputBytes + 4_096, maxOutputTokens);

export const recordStageRun=internalMutation({args:{...executionArgs,roundNumber:v.optional(v.number()),stage:modelStage,provider,model:v.string(),promptVersion:v.string(),schemaVersion:v.string(),finishReason:v.string(),requestHash:v.string(),requestId:v.optional(v.string()),attempt:v.number(),outcome:modelStageOutcome,inputTokens:v.number(),outputTokens:v.number(),now:v.number()},handler:async(ctx,args)=>{const review=await assertReviewParent(ctx.db,args.organizationId,args.reviewId);if(review.headSha!==args.expectedHeadSha||review.executionGeneration!==args.expectedGeneration||review.isStale||!/^\w[\w.:/-]{0,199}$/.test(args.model)||!args.promptVersion||!args.schemaVersion||args.finishReason.length>100||!/^[0-9a-f]{64}$/.test(args.requestHash)||!Number.isInteger(args.attempt)||args.attempt<1||args.attempt>2||![args.inputTokens,args.outputTokens].every(value=>Number.isSafeInteger(value)&&value>=0)||args.provider!==review.provider)throw new ConvexError("model_stage_run_invalid");const prior=(await ctx.db.query("modelStageRuns").withIndex("by_review",q=>q.eq("reviewId",review._id)).collect()).find(item=>item.requestHash===args.requestHash&&item.attempt===args.attempt);if(prior)return;const cost=conservativeModelCost(args.inputTokens,args.outputTokens),consumed=review.budgetConsumed+cost;if(consumed>review.budgetLimit){await ctx.db.patch(review._id,{status:"budget_exhausted",statusReasonCode:"spend_ceiling_reached",budgetCeilingId:"actual-model-usage",nextActionCode:"increase_budget",currentStage:"complete",completedAt:args.now,budgetConsumed:consumed,updatedAt:args.now});throw new ConvexError("budget_exhausted")};await ctx.db.insert("modelStageRuns",{organizationId:args.organizationId,repositoryId:review.repositoryId,reviewId:review._id,...(args.roundNumber===undefined?{}:{roundNumber:args.roundNumber}),stage:args.stage,provider:args.provider,model:args.model,promptVersion:args.promptVersion,schemaVersion:args.schemaVersion,finishReason:args.finishReason,requestHash:args.requestHash,...(args.requestId?{requestId:args.requestId.slice(0,200)}:{}),attempt:args.attempt,outcome:args.outcome,inputTokens:args.inputTokens,outputTokens:args.outputTokens,createdAt:args.now});await ctx.db.insert("usageLedger",{organizationId:args.organizationId,repositoryId:review.repositoryId,reviewId:review._id,kind:"model_tokens",quantity:args.inputTokens+args.outputTokens,unitCost:cost/Math.max(1,args.inputTokens+args.outputTokens),currency:"provider_billed",occurredAt:args.now});await ctx.db.patch(review._id,{budgetConsumed:consumed,updatedAt:args.now})}});

export const preflightStageSpend=internalMutation({args:{...executionArgs,inputBytes:v.number(),maxOutputTokens:v.number(),now:v.number()},handler:async(ctx,args)=>{const review=await assertReviewParent(ctx.db,args.organizationId,args.reviewId);if(review.headSha!==args.expectedHeadSha||review.executionGeneration!==args.expectedGeneration||review.isStale||!Number.isSafeInteger(args.inputBytes)||args.inputBytes<0||!Number.isSafeInteger(args.maxOutputTokens)||args.maxOutputTokens<1)throw new ConvexError("model_stage_budget_invalid");const upperBound=conservativeStageCost(args.inputBytes,args.maxOutputTokens);if(review.budgetConsumed+upperBound<=review.budgetLimit)return{allowed:true as const,upperBound};const previous=await ctx.db.query("reviewEvents").withIndex("by_review",q=>q.eq("reviewId",review._id)).order("desc").first();await ctx.db.patch(review._id,{status:"budget_exhausted",statusReasonCode:"spend_ceiling_reached",budgetCeilingId:"conservative-stage-preflight",nextActionCode:"increase_budget",currentStage:"complete",completedAt:args.now,updatedAt:args.now});await ctx.db.insert("reviewEvents",{organizationId:review.organizationId,reviewId:review._id,sequence:(previous?.sequence??0)+1,type:"status_changed",stage:"complete",internalCode:"model_stage_budget_preflight",metadata:{},createdAt:args.now});return{allowed:false as const,upperBound}}});
