...
export const current = query({
  ...
  handler: async (ctx) => {
    ...
    const repositories = repositoryDocs.filter(item => item.enabled && activeInstallationIds.has(item.installationId)).map(item => ({ id: item._id, installationId: item.installationId, githubRepositoryId: item.githubRepositoryId, owner: item.owner, name: item.name, defaultBranch: item.defaultBranch, visibility: item.visibility ?? "unknown" as const, autofixMode: item.autofixMode, reviewProfile: item.reviewProfile ?? "balanced", reviewTrigger: item.reviewTrigger ?? "manual", reviewPathFilters: item.reviewPathFilters ?? [], paused: Boolean(item.pausedAt), indexState: item.indexState, updatedAt: item.updatedAt }));
