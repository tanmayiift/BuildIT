...
export const compose = internalAction({
  ...
  handler: async (ctx, args): Promise<{ artifactId: Id<"artifacts">; reused: boolean }> => {
    ...
    const body = Buffer.from(composeVerifiedReport({ ...(validation.manager ? { ecosystem: validation.manager } : {}), repository: scope.repository, prNumber: scope.prNumber, headSha: scope.headSha, baseSha: scope.baseSha, configRevision: scope.configRevision,
      coverage: scope.coverage, ...(scope.coverageGap ? { coverageGap: scope.coverageGap } : {}), ...(scope.unreadableSources ? { unreadableSources: scope.unreadableSources } : {}), ...(scope.changeSummary ? { changeSummary: scope.changeSummary } : {}), ...(scope.injectionSurfaces ? { injectionSurfaces: scope.injectionSurfaces } : {}), injectionUnscoped: scope.injectionUnscoped, checks: reportChecks(validation, scope.headSha), findings: analysis.arbitrated, claims: [], evidence: [], environmentAvailable: scope.environmentAvailable, isStale: scope.isStale,
