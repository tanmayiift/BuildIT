...
import { acquireRequirements, describeUnreadableSources, isRequirementSourcePath, repositoryRequirementSources } from "@buildit/orchestrator";
      ...
      for (const [revision, snapshot] of [["head", headSnapshot], ["base", baseSnapshot]] as const) {
        ...
        for (const chunk of chunks) {
          ...
          await ctx.runMutation(internal.reviewArtifactData.complete, { organizationId: scope.organizationId, reviewId: scope.reviewId,
            ...
            artifactId: reserved.artifactId, checksum, size: body.byteLength, coverage, coverageGap, unreadableSources, now: Date.now() });
