...
export const claimExpired = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    for (const artifact of candidates) {
      ...
      if (artifact.deletedAt || artifact.deletionAttempts >= 10 || (artifact.deletionLeaseExpiresAt ?? 0) > args.now || !artifact.reviewId) continue;
      ...
      if (!repository || repository.organizationId !== artifact.organizationId || !review || review.organizationId !== artifact.organizationId || review.repositoryId !== artifact.repositoryId || !artifact.storageKey.startsWith(prefix) || artifact.storageKey.includes("..") || artifact.storageKey.includes("\\")) continue;
...
export const completeDeletion = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(artifact._id, { deletedAt: args.now, deletionLeaseId: undefined, deletionLeaseExpiresAt: undefined, lastDeletionErrorCode: undefined });
    return artifact._id;
  },
});
...
export const failDeletion = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    await ctx.db.patch(artifact._id, { deletionLeaseId: undefined, deletionLeaseExpiresAt: undefined, lastDeletionErrorCode: args.errorCode });
    return artifact._id;
  },
});
