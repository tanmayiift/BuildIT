...
describe("durable validation evidence",()=>{
  ...
  it("derives a ready decision only from complete head evidence",async()=>{const t=convexTest(schema,modules),tenant=await seedTenant(t,"decision","alice"),now=Date.now(),artifactId=await t.run(async ctx=>{await ctx.db.patch(tenant.reviewId,{coverageLevel:"full",status:"validating",currentStage:"analysis"});return ctx.db.insert("artifacts",{organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,reviewId:tenant.reviewId,type:"command_output",storageKey:"decision/validation.json",encrypted:true,checksum:"a".repeat(64),size:10,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0})});await t.run(ctx=>ctx.db.insert("checkRuns",{organizationId:tenant.organizationId,reviewId:tenant.reviewId,kind:"test",nameHash:"b".repeat(64),required:true,status:"completed",conclusion:"passed",commandFingerprint:"c".repeat(64),commitSha:"a".repeat(40),exitCode:0,durationMs:1,artifactId,startedAt:now-1,completedAt:now}));const args={organizationId:tenant.organizationId,reviewId:tenant.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,now};await expect(t.mutation(internal.reviewValidationData.finalizeDecision,args)).resolves.toMatchObject({status:"checks_passed",statusReasonCode:"checks_complete"});await expect(t.mutation(internal.reviewValidationData.finalizeDecision,{...args,now:now+1})).resolves.toMatchObject({status:"checks_passed"});const events=await t.run(ctx=>ctx.db.query("metricEvents").collect());expect(events).toHaveLength(1)});
