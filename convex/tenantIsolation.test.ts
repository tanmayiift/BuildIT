...
describe("Convex tenant isolation", () => {
  ...
  it("selects a repository-scoped model key and records only encrypted analysis metadata", async () => {
    ...
    const seeded = await t.run(async ctx => {
      const contextArtifactId = await ctx.db.insert("artifacts", { organizationId: alpha.organizationId, repositoryId: alpha.repositoryId, reviewId: alpha.reviewId,
    ...
    const checksum = "c".repeat(64), reserved = await t.mutation(internal.reviewModelData.reserveOutput, { ...args, checksum, size: 200, now }),analysisResults={requirements:[{externalIdHash:"1".repeat(64),sourceUrlHash:"2".repeat(64),status:"resolved" as const,confidence:.9}],findings:[{fingerprintHmac:"3".repeat(64),pathHmac:"4".repeat(64),category:"correctness" as const,severity:"high" as const,confidence:.8,blocking:true,evidenceIds:[seeded.contextArtifactId],startLine:1,endLine:2,requirementExternalIdHash:"1".repeat(64),resolution:"open" as const}]};
    await expect(t.mutation(internal.reviewModelData.completeAnalysis, { ...args, organizationId: beta.organizationId, artifactId: reserved.artifactId, checksum, size: 200,
      credentialId: seeded.credentialId, inputTokens: 10, outputTokens: 2, ...analysisResults, now })).rejects.toThrow("parent_scope_mismatch");
    await expect(t.mutation(internal.reviewModelData.completeAnalysis, { ...args, artifactId: reserved.artifactId, checksum, size: 200,
      credentialId: seeded.credentialId, inputTokens: 10, outputTokens: 2, ...analysisResults, now })).resolves.toBe(reserved.artifactId);
    await expect(t.mutation(internal.reviewModelData.completeAnalysis, { ...args, artifactId: reserved.artifactId, checksum, size: 200,
      credentialId: seeded.credentialId, inputTokens: 10, outputTokens: 2, ...analysisResults, now: now + 1 })).resolves.toBe(reserved.artifactId);
    const stored = await t.run(async ctx => ({ artifact: await ctx.db.get(reserved.artifactId), credential: await ctx.db.get(seeded.credentialId), usage: await ctx.db.query("usageLedger").withIndex("by_review", q => q.eq("reviewId", alpha.reviewId)).collect(),requirements:await ctx.db.query("requirements").withIndex("by_review",q=>q.eq("reviewId",alpha.reviewId)).collect(),findings:await ctx.db.query("findings").withIndex("by_review_severity",q=>q.eq("reviewId",alpha.reviewId).eq("severity","high")).collect() }));
    ...
    expect(stored.requirements).toHaveLength(1);expect(stored.findings).toHaveLength(1);expect(stored.findings[0]).toMatchObject({blocking:true,resolution:"open",contentArtifactId:reserved.artifactId,evidenceIds:[seeded.contextArtifactId]});
...
describe("durable validation evidence",()=>{
  ...
  it("derives a ready decision only from complete head evidence",async()=>{const t=convexTest(schema,modules),tenant=await seedTenant(t,"decision","alice"),now=Date.now(),{artifactId,reportArtifactId}=await t.run(async ctx=>{await ctx.db.patch(tenant.reviewId,{coverageLevel:"full",status:"validating",currentStage:"analysis"});const artifactId=await ctx.db.insert("artifacts",{organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,reviewId:tenant.reviewId,type:"command_output",storageKey:"decision/validation.json",encrypted:true,checksum:"a".repeat(64),size:10,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0});const reportArtifactId=await ctx.db.insert("artifacts",{organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,reviewId:tenant.reviewId,type:"review_message",storageKey:"decision/report.md",encrypted:true,checksum:"d".repeat(64),size:10,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0});return{artifactId,reportArtifactId}});await t.run(ctx=>ctx.db.insert("checkRuns",{organizationId:tenant.organizationId,reviewId:tenant.reviewId,kind:"test",nameHash:"b".repeat(64),required:true,status:"completed",conclusion:"passed",commandFingerprint:"c".repeat(64),commitSha:"a".repeat(40),exitCode:0,durationMs:1,artifactId,startedAt:now-1,completedAt:now}));const args={organizationId:tenant.organizationId,reviewId:tenant.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,reportArtifactId,now};await expect(t.mutation(internal.reviewValidationData.finalizeDecision,args)).resolves.toMatchObject({status:"checks_passed",statusReasonCode:"checks_complete"});await expect(t.mutation(internal.reviewValidationData.finalizeDecision,{...args,now:now+1})).resolves.toMatchObject({status:"checks_passed"});const events=await t.run(ctx=>ctx.db.query("metricEvents").collect());expect(events).toHaveLength(1);const reviewEvent=await t.run(ctx=>ctx.db.query("reviewEvents").withIndex("by_review",q=>q.eq("reviewId",tenant.reviewId)).filter(q=>q.eq(q.field("sequence"),5)).unique());expect(reviewEvent?.publicMessageArtifactId).toBe(reportArtifactId)});

  it("rejects another tenant's final report artifact",async()=>{const t=convexTest(schema,modules),alpha=await seedTenant(t,"report-alpha","alice"),beta=await seedTenant(t,"report-beta","bob"),now=Date.now(),foreignReportId=await t.run(ctx=>ctx.db.insert("artifacts",{organizationId:beta.organizationId,repositoryId:beta.repositoryId,reviewId:beta.reviewId,type:"review_message",storageKey:"report-beta/report.md",encrypted:true,checksum:"d".repeat(64),size:10,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0}));await t.run(ctx=>ctx.db.patch(alpha.reviewId,{coverageLevel:"full",status:"validating",currentStage:"analysis"}));await expect(t.mutation(internal.reviewValidationData.finalizeDecision,{organizationId:alpha.organizationId,reviewId:alpha.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,reportArtifactId:foreignReportId,now})).rejects.toThrow("report_artifact_mismatch")});
...
describe("durable Autofix evidence",()=>{it("records one exact candidate round idempotently and rejects foreign artifacts",async()=>{const t=convexTest(schema,modules),alpha=await seedTenant(t,"autofix-alpha","alice"),beta=await seedTenant(t,"autofix-beta","bob"),now=Date.now(),{patchArtifactId,validationArtifactId}=await t.run(async ctx=>{await ctx.db.patch(alpha.reviewId,{mode:"autofix",status:"validating",currentStage:"analysis"});const patchArtifactId=await ctx.db.insert("artifacts",{organizationId:alpha.organizationId,repositoryId:alpha.repositoryId,reviewId:alpha.reviewId,type:"patch",storageKey:"autofix-alpha/autofix-1-candidate-0.json",encrypted:true,checksum:"a".repeat(64),size:10,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0}),validationArtifactId=await ctx.db.insert("artifacts",{organizationId:alpha.organizationId,repositoryId:alpha.repositoryId,reviewId:alpha.reviewId,type:"command_output",storageKey:"autofix-alpha/autofix-1-validation.json",encrypted:true,checksum:"b".repeat(64),size:10,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0});return{patchArtifactId,validationArtifactId}}),summary={commitSha:"c".repeat(40),planId:"test",kind:"test" as const,required:true,conclusion:"passed" as const,exitCode:0,durationMs:1,commandFingerprint:"d".repeat(64),nameHash:"e".repeat(64)},args={organizationId:alpha.organizationId,reviewId:alpha.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,roundNumber:1,candidateCommitSha:"c".repeat(40),patchFingerprint:"f".repeat(64),patchArtifactId,validationArtifactId,summaries:[summary],outcome:"passed" as const,now};const roundId=await t.mutation(internal.reviewAutofixData.completeRound,args);await expect(t.mutation(internal.reviewAutofixData.completeRound,{...args,now:now+1})).resolves.toBe(roundId);expect(await t.run(ctx=>ctx.db.get(alpha.reviewId))).toMatchObject({status:"autofixing",completedRoundCount:1,patchAttemptCount:1});const foreign=await t.run(ctx=>ctx.db.insert("artifacts",{organizationId:beta.organizationId,repositoryId:beta.repositoryId,reviewId:beta.reviewId,type:"patch",storageKey:"foreign",encrypted:true,checksum:"1".repeat(64),size:1,redactionStatus:"redacted",expiresAt:now+60_000,deletionAttempts:0}));await expect(t.mutation(internal.reviewAutofixData.completeRound,{...args,roundNumber:2,patchArtifactId:foreign,now:now+2})).rejects.toThrow("autofix_round_mismatch")});it("marks delivery complete only after a passed round, encrypted report, and all four GitHub effects",async()=>{const t=convexTest(schema,modules),tenant=await seedTenant(t,"autofix-delivery","alice"),now=Date.now(),{patchArtifactId,validationArtifactId,reportArtifactId}=await t.run(async ctx=>{await ctx.db.patch(tenant.reviewId,{mode:"autofix",status:"validating",currentStage:"analysis"});const base={organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,reviewId:tenant.reviewId,encrypted:true as const,size:10,redactionStatus:"redacted" as const,expiresAt:now+60_000,deletionAttempts:0};return{patchArtifactId:await ctx.db.insert("artifacts",{...base,type:"patch",storageKey:"delivery/autofix-1-candidate-0.json",checksum:"a".repeat(64)}),validationArtifactId:await ctx.db.insert("artifacts",{...base,type:"command_output",storageKey:"delivery/autofix-1-validation.json",checksum:"b".repeat(64)}),reportArtifactId:await ctx.db.insert("artifacts",{...base,type:"review_message",storageKey:"delivery/autofix-1-handoff.md",checksum:"c".repeat(64)})}}),candidateCommitSha="d".repeat(40),roundArgs={organizationId:tenant.organizationId,reviewId:tenant.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,roundNumber:1,candidateCommitSha,patchFingerprint:"e".repeat(64),patchArtifactId,validationArtifactId,summaries:[{commitSha:candidateCommitSha,planId:"test",kind:"test" as const,required:true,conclusion:"passed" as const,exitCode:0,durationMs:1,commandFingerprint:"f".repeat(64),nameHash:"1".repeat(64)}],outcome:"passed" as const,now};await t.mutation(internal.reviewAutofixData.completeRound,roundArgs);for(const [index,type] of (["branch_create","stacked_pr_create","check_update","comment_update"] as const).entries())await t.run(ctx=>ctx.db.insert("githubSideEffects",{organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,reviewId:tenant.reviewId,operationKey:`delivery-${index}`,type,requestHash:"2".repeat(64),status:"completed",externalId:String(index+1),createdAt:now,updatedAt:now}));const deliveryArgs={organizationId:tenant.organizationId,reviewId:tenant.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,roundNumber:1,candidateCommitSha,reportArtifactId,now:now+1};await expect(t.mutation(internal.reviewAutofixData.completeDelivery,deliveryArgs)).resolves.toBe(tenant.reviewId);await expect(t.mutation(internal.reviewAutofixData.completeDelivery,{...deliveryArgs,now:now+2})).resolves.toBe(tenant.reviewId);expect(await t.run(ctx=>ctx.db.get(tenant.reviewId))).toMatchObject({status:"delivered",nextActionCode:"human_merge",githubCheckConclusion:"success"});expect(await t.run(ctx=>ctx.db.query("metricEvents").collect())).toHaveLength(1)})});
...
describe("GitHub webhook durability",()=>{
  ...
  it("materializes a pinned command into exactly one tenant-scoped review",async()=>{const t=convexTest(schema,modules),tenant=await seedTenant(t,"materialize","alice"),delivery=await t.mutation(internal.githubWebhookData.reserve,{deliveryId:"delivery-review",event:"issue_comment",action:"created",installationId:20,disposition:"processed",now:1}),snapshot={deliveryId:"delivery-review",prNumber:7,headSha:"a".repeat(40),baseSha:"b".repeat(40),headRefHash:"c".repeat(64),baseRefHash:"d".repeat(64),isFork:false,triggerVerb:"review" as const};await t.mutation(internal.githubWebhookData.recordPinnedSnapshot,snapshot);const args={deliveryId:"delivery-review",organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,baseRef:"main",triggerActor:"e".repeat(64),actorPermission:"admin" as const,now:2},created=await t.mutation(internal.githubWebhookData.materializeReview,args);expect(created).toMatchObject({status:"queued",headSha:"a".repeat(40),executionGeneration:0});expect(await t.run(ctx=>ctx.db.get(created.reviewId))).toMatchObject({organizationId:tenant.organizationId,repositoryId:tenant.repositoryId,prNumber:7,headSha:"a".repeat(40),status:"queued",trigger:"github_comment",triggerActor:"e".repeat(64)});expect(await t.run(ctx=>ctx.db.get(delivery.id))).toMatchObject({reviewId:created.reviewId});await expect(t.mutation(internal.githubWebhookData.materializeReview,args)).rejects.toThrow("review_request_not_materializable");expect(await t.run(ctx=>ctx.db.query("reviewLocks").withIndex("by_scope",q=>q.eq("repositoryId",tenant.repositoryId).eq("prNumber",7).eq("headSha","a".repeat(40)).eq("mode","review")).collect())).toHaveLength(1)});
  it("does not start a blocked, stale, or replaced review workflow",async()=>{const t=convexTest(schema,modules),tenant=await seedTenant(t,"not-runnable","alice"),base={organizationId:tenant.organizationId,reviewId:tenant.reviewId,expectedHeadSha:"a".repeat(40),expectedGeneration:0,now:1};await t.run(ctx=>ctx.db.patch(tenant.reviewId,{status:"blocked"}));await expect(t.mutation(internal.durableReview.start,base)).rejects.toThrow("review_not_runnable");await t.run(ctx=>ctx.db.patch(tenant.reviewId,{status:"queued",isStale:true}));await expect(t.mutation(internal.durableReview.start,base)).rejects.toThrow("review_not_runnable");await t.run(ctx=>ctx.db.patch(tenant.reviewId,{isStale:false,executionGeneration:1}));await expect(t.mutation(internal.durableReview.start,base)).rejects.toThrow("review_not_runnable")});
