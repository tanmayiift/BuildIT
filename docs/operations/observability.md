...
The versioned operator template is `observability/grafana/notification-templates/buildit-operator-v1.tmpl`. It changes only BuildIT's alert content; Grafana still owns the outer mail frame. Validate the fixed stack and template without a credential using `node scripts/provision-buildit-grafana-template.mjs --dry-run`. A BuildIT-scoped Grafana service-account secret may be supplied through `BUILDIT_GRAFANA_SERVICE_ACCOUNT_TOKEN` to publish that exact template name. Never pass the secret as a command argument or store it in a shell-history file. The script refuses every stack hostname except the approved BuildIT stack and writes only `buildit-operator-v1`; contact routing remains a separate human-reviewed operation so shared Orbit routing cannot be changed by this script.
